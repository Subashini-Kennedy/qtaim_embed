...
#SBATCH -p gpu_p6
#SBATCH -C h100
#SBATCH -J train_sota_qtaim_corrected
#SBATCH -t 48:00:00
...
# Run outputs
PERSIST_RUN_DIR="/lustre/fsn1/projects/rech/ihj/$USER/qtaim_embed_private/logs_suba/train_43k_qtaim_corrected${SLURM_JOB_ID}"
...
# Config file
CFG="/lustre/fsn1/projects/rech/ihj/$USER/qtaim_embed_private/configs_suba/settings_qm9_nonlmdb_43k_withval_qtaim.json"
if [[ ! -f "$CFG" ]]; then
...
srun --ntasks-per-node=1 --cpu-bind=none python -u \
  ...
  -config "${CFG}" \
  -project_name "qm9_43k_qtaim" \
  -log_save_dir "${LOG_DIR}" \
  -dataset_test_loc "/lustre/fsn1/projects/rech/ihj/$USER/qtaim_embed_private/data_suba/filtered_qtaim_fullqm9_corrected/test_43k.pkl" \
  -model_name "qtaim" \
